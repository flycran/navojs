...
Navo controls menu visibility via `authenticat`, but routes generated by `generateRoutes` always include all pages. To block unauthorized URL access, use `useMatchedNodes` in your layout — if no node matches, the current route is unauthorized:
...
This works because `useMatchedNodes` looks up the current route in `authedIdMap` — routes filtered out by `authenticat` will never match, giving you a single `!matchedNodes.length` check for the entire route guard.

> **Design note**: Navo intentionally does not ship a built-in 403 component. It provides the primitives (`authedIdMap`, `useMatchedNodes`, `hasCanAccess`) and leaves the UI decision to you.
