...
export const useMatchedNodes = () => {
  const navo = useContext(NavoContext)
  if (!navo) throw new Error('useMatchedNodes must be used within a NavoProvider.')
  const matches = useMatches()

  return useMemo(() => {
    const matchedNodes: NavoNode[] = []
    const matchedIds: string[] = []
    // TanStack Router 的 matches 是从上到下的顺序，需要倒序遍历
    for (let i = matches.length - 1; i >= 0; i--) {
      const element = matches[i]
      // TanStack Router 使用 routeId 而不是 id
      const routeId = element.routeId
      const node = navo.authedIdMap.get(routeId)
...
export const useNavo = (): UseNavoReturn => {
  const navo = useContext(NavoContext)
  if (!navo) throw new Error('useNavo must be used within a NavoProvider.')
